...
class PdfExporterTest {
    ...
    fun exportsTwoImagesToATwoPagePdf(): Unit = runBlocking {
        ...
        val result = PdfExporter.export(context, pages, Uri.fromFile(out), ExportOptions())
        ...
        val pageCount = Regex("/Type\\s*/Page[^s]").findAll(text).count()
        assertEquals("two /Type /Page entries", 2, pageCount)
        out.delete()
    }
}
